...
if (bundle.mcpConfig) {
  ...
  const merged = {
    ...existingConfig,
    mcpServers: { ...existingServers, ...bundle.mcpConfig.mcpServers },
  }
...
```typescript
export async function writeWindsurfBundle(
  outputRoot: string,
  bundle: WindsurfBundle
): Promise<void> {
...
if (scopeValue !== undefined) {
  ...
  if (!isTargetScope(scopeValue) || !target.supportedScopes.includes(scopeValue)) {
    throw new Error(
      `Target "${targetName}" does not support --scope ${scopeValue}. Supported: ${target.supportedScopes.join(', ')}`
    )
  }
}
